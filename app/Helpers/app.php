...
if (!function_exists('notify')) {
    function notify($data = ['status' => 'success', 'text' => 'Success', 'title' => 'Success'])
    {
        request()->session()->flash('notify', $data);
    }
}
...
if (!function_exists('toRp')) {
    function toRp($parm, $dec = 0)
    {
        return 'Rp. ' . number_format(floatval($parm), $dec, ',', '.');
    }
}
